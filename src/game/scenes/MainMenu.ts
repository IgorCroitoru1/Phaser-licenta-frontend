...
export  class MainMenu extends Phaser.Scene{
    ...
    currentRoomId: any;
    constructor() {
        super({ key: 'MainScene', physics: { arcade: { debug: true } } }); // ✅ Enable Physics
    }
    ...
    create(){
       ...
       const flowersTile = this.map.addTilesetImage("flowers", "flowers");
       // const wallsTileset = this.map.addTilesetImage("wall", "wall");
        this.graphics = this.add.graphics();
       
          if(!xfloorTileset || !flowersTile ){
            return
          }
        const xfloorLayer = this.map.createLayer("floor", [xfloorTileset,], 0,0);
        const wallsLayer = this.map.createLayer("walls", flowersTile, 0,0);
         // const testFloor = this.map.createLayer("wood-floor-smaller", xfloorTileset, 0,0);
       xfloorLayer?.setCullPadding(64,64);
        xfloorLayer?.renderDebug(this.add.graphics());
         //xfloorLayer?.setPosition(0, -80);
